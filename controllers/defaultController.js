...
module.exports = {
    ...
    getPostsByCategory: async (req, res) => {
        const categories = await Category.find().lean();
        const titre =await Category.find({_id:ObjectId(req.params.id)}).lean()
        
        Post.find({category:ObjectId(req.params.id)}).lean()
            .populate('posts')
            .then(posts => {
                
                res.render('default/category', {posts: posts, categories : categories, title : titre[0].title});
    ...
    getTarif : (req, res) =>{
        ...
        .populate('prices')
        .then(prices =>{       
             res.render('default/tarif', {prices : prices})
            })
